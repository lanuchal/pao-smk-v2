<div class="bg-dark overflow-hidden" id="sidebar-wrapper">
    ...
    <ul class="sidebar-nav overflow-auto" style="margin-top: 80px; max-height: 90vh; padding-bottom: 30px">
        ...
        <li class="px-2">
            ...
            <a class="rounded <?= (defending($_GET['p']) == 'setroom') ? "active" : "" ?> " href="?s=<?= defending($_GET['s']) ?>&p=setroom">
                <div class="d-flex align-items-center">
                    <i class='bx bxs-layer'></i>
                    <span class="">ตั้งค่าห้องประชุม</span>
                </div>
            </a>
            <a class="rounded <?= (defending($_GET['p']) == 'formatroom') ? "active" : "" ?> " href="?s=<?= defending($_GET['s']) ?>&p=formatroom">
                <div class="d-flex align-items-center">
                    <i class='bx bxs-layer'></i>
                    <span class="">ตั้งค่ารูปแบบห้องประชุม</span>
                </div>
            </a>
			<a class="rounded <?= (defending($_GET['p']) == 'setdeviceroom') ? "active" : "" ?> " href="?s=<?= defending($_GET['s']) ?>&p=setdeviceroom">
                <div class="d-flex align-items-center">
                    <i class='bx bxs-layer'></i>
                    <span class="">ตั้งค่าอุปกรณ์</span>
                </div>
            </a>
        </li>
    </ul>
