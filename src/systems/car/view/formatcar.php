            ...
            <form method="POST" action="?s=<?= defending($_GET['s']) ?>&p=<?= defending($_GET['p']) ?>">
                <div class="modal-header border-0">
                    <h1 class="modal-title fs-5" id="addModalLabel">
                        <div class="d-flex align-items-center"> <i class='bx bxs-plus-circle me-2'></i> เพิ่มประเภทรถ </div>
                    </h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body border-0">
                    <div class="mb-3">
                        <label for="name_style" class="form-label">ชื่อประเภทรถ</label>
        ...
        <div class="col-md-4">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                <div class="d-flex align-items-center">
                    <i class='bx bxs-plus-circle me-2'></i> เพิ่มประเภทรถ
                </div>
            </button>
        </div>
    </div>
    ...
    <table class="table table-hover" id="table_data">
        <thead>
            <tr>
                ...
                <th class="text-center">ชื่อประเภทของรถ</th>
                            ...
                            <form method="POST" action="?s=<?= defending($_GET['s']) ?>&p=<?= defending($_GET['p']) ?>">
                                <div class="modal-header border-0">
                                    <h1 class="modal-title fs-5" id="editModalLabel<?= $row['id_style']; ?>">
                                        <div class="d-flex align-items-center"> <i class='bx bxs-edit me-2'></i> แก้ไขประเภทรถ</div>
                                    </h1>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body border-0">
                                    <div class="mb-3">
                                        ...
                                        <label for="name_style" class="form-label">ชื่อประเภทของรถ</label>
                                        <input type="text" class="form-control" name="name_style" placeholder="ชื่อประเภทของรถ" value="<?= $row['name_style']; ?>" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="position" class="form-label">ตำแหน่งการเรียงลำดับ</label>
                            ...
                            <form method="POST" action="?s=<?= defending($_GET['s']) ?>&p=<?= defending($_GET['p']) ?>">
                                <div class="modal-header border-0">
                                    <h1 class="modal-title fs-5" id="deleteModalLabel<?= $row['id_style']; ?>">
                                        <div class="d-flex align-items-center"> <i class='bx bx-trash me-2'></i> ลบประเภทรถ </div>
                                    </h1>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body border-0">
                                    <div class="mb-3">
                                        <input type="hidden" name="id_style" value="<?= $row['id_style']; ?>">
                                        <label for="name_style" class="form-label">ชื่อประเภทของรถ</label>
                                        <input type="text" class="form-control" name="name_style" placeholder="ชื่อประเภทของรถ" value="<?= $row['name_style']; ?>" disabled>
                                    </div>
                                </div>
                                <div class="modal-footer border-0">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ปิด</button>
